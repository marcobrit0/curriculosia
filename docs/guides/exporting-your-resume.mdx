...
Currículos IA lets you export your resume in two formats: **PDF** and **JSON**. Each format serves a different purpose—PDFs are ideal for sharing with recruiters and uploading to job applications, while JSON files are perfect for backups, importing into other tools, or working with AI assistants.
...
<AccordionGroup>
  <Accordion title="Backup and version control" icon="floppy-disk">
    Keep local backups of your resume data. If you ever need to restore or compare versions, you can import the JSON file back into Currículos IA.
  </Accordion>

<Accordion title="Import into Currículos IA" icon="file-import">
  You can import a JSON export back into Currículos IA to create a new resume based on that data. This is useful for
...
<Steps>
  ...
  <Step title="Import the modified JSON back into Currículos IA">
    1. Go to your **Dashboard**
    2. Click **Import an existing resume**
    3. Select **Currículos IA (JSON)** as the type
...
The exported JSON follows the Currículos IA schema. Here's a high-level overview of the structure:
...
```json
{
  ...
  "properties": {
    ...
    "version": { "type": "string", "const": "5.0.0", "description": "The version of the Currículos IA JSON Schema" },
    "$schema": {
      "type": "string",
      "const": "http://localhost:3000/schema.json",
      "description": "The URL of the Currículos IA JSON Schema, used for validation and documentation purposes."
    }
  },
...
<Tip>
  ...
  [`https://curriculos.ia.br/schema.json`](https://curriculos.ia.br/schema.json). Referencing this is helpful for
  validating your resume JSON or providing a detailed schema reference when collaborating with AI assistants.
</Tip>

## Re-importing your JSON

To import a JSON file back into Currículos IA:
...
<Step title="Select the right format">
  In the format options, select **Currículos IA (JSON)** if your file was exported from the current version.
</Step>
