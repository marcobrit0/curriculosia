...
description: "Connect Currículos IA to AI tools like Claude Desktop, Cursor, and Codex using the Model Context Protocol"
---

The Currículos IA MCP server lets you manage and modify your resumes through any MCP-compatible AI tool — Claude Desktop, Cursor, Codex, and more. It connects to the Currículos IA API and exposes tools for listing, reading, and patching resumes using natural language.
...
<Steps>
  <Step title="Choose your authentication method">
    Currículos IA MCP supports two authentication methods:
  ...
  <Step title="If using API key, create one">
    Head over to [https://curriculos.ia.br](https://curriculos.ia.br) (or your self-hosted instance), sign in, and navigate to **Settings → API Keys**. Click **Create a new API key**, give it a name, and copy the secret — it's only shown once.
...
```json
{
  "mcpServers": {
    "curriculosia": {
      "url": "https://curriculos.ia.br/mcp"
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "curriculosia": {
      "command": "npx",
      "args": ["mcp-remote", "https://curriculos.ia.br/mcp", "--header", "x-api-key:your-api-key"]
    }
  }
}
```
...
## Authentication Details (How Currículos IA MCP Works)

Currículos IA MCP accepts authentication in this order:
...
Currículos IA is configured as an OAuth authorization server for MCP clients:

- The MCP endpoint is `https://curriculos.ia.br/mcp`.
...
```bash
codex mcp add curriculosia --url https://curriculos.ia.br/mcp
```

Then log in with OAuth:

```bash
codex mcp login curriculosia
```

API key fallback (`config.toml`):

```toml
[mcp_servers."curriculosia"]
url = "https://curriculos.ia.br/mcp"
...
Add `https://curriculos.ia.br/mcp` as a custom remote MCP connector, then connect with OAuth in Claude's connector UI.
...
If you're running a self-hosted Currículos IA instance, replace `https://curriculos.ia.br/mcp` with your instance URL:
...
| "API error (403)"                                             | The resume is locked. Unlock it in the Currículos IA dashboard                                                                 |
