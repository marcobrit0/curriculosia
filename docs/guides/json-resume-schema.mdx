...
description: "Learn about the Currículos IA JSON Schema, available at /schema.json, and how it aims to become the gold standard for portable resume data."
---
...
Currículos IA aims to be the **gold standard in JSON Resume Schemas**. A universal, well-documented, and rigorously typed schema that any service can adopt to enable seamless resume portability across the entire job application ecosystem.

Imagine:

- Exporting your resume from Currículos IA as a JSON file
...
The Currículos IA JSON Schema is publicly available at:

<Info>[https://curriculos.ia.br/schema.json](https://curriculos.ia.br/schema.json)</Info>

This endpoint returns a complete JSON Schema (draft 07 compatible) that describes the structure of a Currículos IA document. It includes:
...
```json
{
  "$schema": "https://curriculos.ia.br/schema.json",
  "version": "5.0.0"
}
```

When importing resumes, Currículos IA uses this version to handle migrations and ensure backward compatibility.
...
const ajv = new Ajv();
const schema = await fetch("https://curriculos.ia.br/schema.json").then((r) => r.json());
...
- **Job portals** can accept Currículos IA JSON files directly
...
Below is the complete JSON Schema for Currículos IA. You can also fetch the latest version directly from [https://curriculos.ia.br/schema.json](https://curriculos.ia.br/schema.json).
