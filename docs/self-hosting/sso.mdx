...
Currículos IA supports custom OAuth providers, allowing you to integrate with enterprise identity providers and self-hosted authentication solutions. This is particularly useful for organizations that want to:
...
<Info>
  ...
  [curriculos.ia.br](https://curriculos.ia.br), you can use the built-in Google and GitHub sign-in options.
</Info>
...
<Tabs>
  <Tab title="Option A: OIDC Discovery (Recommended)">
    ...
    **Examples:**
    - Authentik: `https://auth.example.com/application/o/curriculosia/.well-known/openid-configuration`
...
Currículos IA automatically maps user profile data from the OAuth provider. The following fields are used:

| Currículos IA Field  | OAuth Profile Fields (in order of preference) |
| -------------------- | --------------------------------------------- |
...
OAUTH_DISCOVERY_URL="https://auth.example.com/application/o/curriculosia/.well-known/openid-configuration"
```
  </Step>
</Steps>
...
identity_providers:
  oidc:
    clients:
      - client_id: curriculosia
        client_name: Currículos IA
...
OAUTH_CLIENT_ID="curriculosia"
OAUTH_CLIENT_SECRET="your-plain-secret"
OAUTH_DISCOVERY_URL="https://auth.example.com/.well-known/openid-configuration"
```

    <Warning>
      Use the **plain text** secret in Currículos IA's environment, not the hashed version used in Authelia's configuration.
    </Warning>
...
<Steps>
  <Step title="Create a client">
    ...
    3. Set **Client ID** (e.g., `curriculosia`)
...
OAUTH_CLIENT_ID="curriculosia-client-id"
OAUTH_CLIENT_SECRET="curriculosia-client-secret"
OAUTH_DISCOVERY_URL="https://auth.company.com/application/o/curriculosia/.well-known/openid-configuration"
...
<CardGroup cols={2}>
  <Card title="Use HTTPS" icon="lock">
    Always use HTTPS for both your Currículos IA instance and OAuth provider in production. OAuth tokens should never be
  ...
  <Card title="Review scopes" icon="list-check">
    Only request the scopes you need. The default (`openid profile email`) is sufficient for Currículos IA.
  </Card>
</CardGroup>
