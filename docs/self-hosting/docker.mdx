...
description: "A comprehensive guide to self-host Currículos IA with Docker (Postgres + Printer), including a detailed environment variable reference and troubleshooting tips."
---

## Overview

Currículos IA can be self-hosted using Docker in a matter of minutes, and this guide will walk you through the process. Here are some of the services you'll need to get started:
...
- Docker Hub: `marcobrit0/curriculosia:latest`
- GitHub Container Registry: `ghcr.io/marcobrit0/curriculosia:latest`
...
Create a new folder (for example `curriculosia/`) with:
...
# to let the browser navigate to a non-public instance of Currículos IA
...
```
  ...
  <Step title="Create compose.yml">
    This setup runs Postgres + Printer + Currículos IA on a private Docker network.
...
services:
  ...
  curriculosia:
    image: marcobrit0/curriculosia:latest
    # image: ghcr.io/marcobrit0/curriculosia:latest
    restart: unless-stopped
    ports:
      - "3000:3000"
    env_file:
...
```
    ...
    <Tip>
      Prefer pulling from Docker Hub? Keep <code>marcobrit0/curriculosia:latest</code>. Prefer GHCR? Swap it to <code>ghcr.io/marcobrit0/curriculosia:latest</code>.
    </Tip>
...
```bash
docker compose logs -f curriculosia
```

    </CodeGroup>

    Currículos IA should now be available at your `APP_URL` (for the example above: `http://localhost:3000`).
...
<AccordionGroup>
  ...
  <Accordion title="Printer">
    - **`PRINTER_ENDPOINT`**: Base URL where Currículos IA reaches the printer service. In Compose: `http://printer:3000`. If using browserless with token authentication, include the token as a query parameter: `ws://printer:3000?token=your-secret-token`.
...
```
  ...
  <Accordion title="Email (SMTP, optional)">
    ...
    - **`SMTP_FROM`**: Default from address (for example, `Currículos IA <noreply@curriculos.ia.br>`).
...
To update your Currículos IA installation to the latest available version, follow these steps:
...
Regular backups are essential to protect your data. Currículos IA stores data in two places: the PostgreSQL database and file uploads (either local storage or S3).
...
Currículos IA exposes a health check endpoint at `/api/health` that verifies the application and its dependencies are functioning correctly. If any critical service (such as the database connection) fails, the health check will return an unhealthy status.
...
This is particularly useful in high-availability setups where you have multiple instances of Currículos IA. If one instance becomes unhealthy (for example, it loses its database connection), the reverse proxy will stop routing traffic to it until it recovers.
...
You can manually verify the health of your Currículos IA instance:
...
<AccordionGroup>
  <Accordion title="The app container exits immediately">
    ...
    - **What to do**:  
      When the app container exits right away, you'll want to check the logs for more information about the error. Run the following command to view real-time logs from the Currículos IA container:
      ```bash
      docker compose logs -f curriculosia
      ```
  </Accordion>
...
<Accordion title="PDF export fails / printing is stuck">
  - **Common cause**: Currículos IA can't reach the printer or the printer can't reach your app. - **Checks**: -
